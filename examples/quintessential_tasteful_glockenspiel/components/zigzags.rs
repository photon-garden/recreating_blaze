...
pub fn render(params: &mut RenderParams) {
    ...
    for zigzag in zigzags {
        ...
        params.draw.polygon().points(points).color(riley_black());
    }
}

fn zigzag_polygon(app: &App, center_angle: f32, circles: &[riley_circles::Circle]) -> Path2 {
    // let offset = app.normalized_mouse_x().denormalize(0.01, 0.1);
    // dbg!(offset);
    let offset = 0.04;

    let offsets = vec![
        0.0, offset, 0.00, offset, 0.00, offset, 0.00, offset, offset,
    ];
