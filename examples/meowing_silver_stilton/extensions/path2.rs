...
pub trait Path2Extension {
    ...
    fn expand_symmetrically(&self, magnitudes: &Vec<f32>) -> Path2;
    fn expand(&self, magnitudes: &Vec<(f32, f32)>) -> Path2;
...
impl Path2Extension for Path2 {
    ...
    fn expand_symmetrically(&self, magnitudes: &Vec<f32>) -> Path2 {
        let expanded_magnitudes: Vec<_> = magnitudes
            .iter()
            .map(|magnitude| (*magnitude, *magnitude))
            .collect();

        self.expand(&expanded_magnitudes)
    }
    fn expand(&self, magnitudes: &Vec<(f32, f32)>) -> Path2 {
        let mut left_points = Vec::with_capacity(self.len());
        let mut right_points = Vec::with_capacity(self.len());

        if magnitudes.len() != self.len() {
            panic!("Tried to expand a path, but magnitudes had the wrong number of elements.");
        }
        ...
        for i in 0..self.len() {
            ...
            let (left_magnitude, right_magnitude) = magnitudes.get(i).unwrap();
